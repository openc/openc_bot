...
module OpencBot
  ...
  def ocdata_field_names
    sqlite_magic_connection.execute("PRAGMA table_info(ocdata)").collect { |c| c["name"] }
  end

  def table_summary
    select_sql = "COUNT(1) Total, " + ocdata_field_names.collect { |fn| "COUNT(#{fn}) #{fn}_not_null" }.join(", ") + " FROM ocdata"
