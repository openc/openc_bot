  ...
  module Helpers
    module AlphaSearch
      ...
      def numbers_of_chars_in_search
        const_defined?("NUMBER_OF_CHARS_IN_SEARCH") ? const_get("NUMBER_OF_CHARS_IN_SEARCH") : 1
      end

      def search_for_entities_for_term(_term, _options = {})
      ...
      def get_results_and_extract_data_for(prefix, search_offset)
        while search_offset
          ...
          if response =~ /webservices\/HRG/ # check has links to companies
